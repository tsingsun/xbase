...
//
// By default decode looks for "dbf" tag, but this can be changed by setting
// Decoder.Tag field.
//
// To Decode into a custom type v must implement xbase.Unmarshaler or
// encoding.TextUnmarshaler.
//
// Anonymous struct fields with tags are treated like normal fields and they
// must implement xbase.Unmarshaler or encoding.TextUnmarshaler unless inline
// tag is specified.
//
...
func (d *Decoder) NormalizeHeader(f func(string) string) error {
	...
	if len(set) != len(d.header) {
		return errors.New("xbase: normalize header results in conflicting columns")
	}
...
func (d *Decoder) Register(f interface{}) {
	...
	if argType.Kind() == reflect.Interface && argType.NumMethod() == 0 {
		panic("xbase: func argument type must not be an empty interface")
	}
	...
	if _, ok := d.funcMap[argType]; ok {
		panic("xbase: func " + typ.String() + " already registered")
	}
...
// that is available e.g. in dbf.Reader (since Go1.17).
//
